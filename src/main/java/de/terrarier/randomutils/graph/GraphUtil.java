...
public class GraphUtil {
    ...
    private static String buildScaledGraphVert(int x, int y, int[] dataPoints, GraphDifferenceVisualizationMode mode,
        ...
        for (int i = 0; i < dataPoints.length; i++) {
            ...
            final int diff = diff(lastYRows, i == dataPoints.length - 1 ? yRows : dataPoints[i + 1], yRows, mode);
            insertEntryToGraph(graph[row], subIndex, yRows, diff);
            lastYRows = yRows;
        }
        final StringBuilder sb = new StringBuilder();
    ...
    private static int diff(int lastRows, int nextRows, int currentRows, GraphDifferenceVisualizationMode mode) {
        switch (mode) {
            case DECAY:
